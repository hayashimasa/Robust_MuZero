...
This script defines the SelfPlay class for the MuZero algorithm. It also
contains an implementation of Monte Carlo Tree Search.
...
Put this file in place of self_play.py, for the most easy and straightforward robustness model (random white noise applied on the choice of the agent)

Change in the MCTS class the robustness to False for the main version 
...
class MCTS:
    ...
    def run(
        ...
        for _ in range(self.config.num_simulations):
            ...
            while node.expanded():
                ...
                action, node = self.select_child(node, min_max_stats , self.robustness)
    ...
    def select_child(self, node, min_max_stats, rob ):
        ...
        if score == "UCB" :
            ...
            val_scores = [self.raw_score(node, child, min_max_stats)
                for action, child in node.children.items()]
            rnd_noise = numpy.random.normal(0,1,len(val_scores))
            max_rd = max(val_scores+rnd_noise)
            action = numpy.random.choice(
                [
                    action
                    for i, (action, child) in enumerate(node.children.items())
                    if val_scores[i] + rnd_noise[i] == max_rd
                ]
            )
    ...
    def raw_score(self, parent , child , min_max_stats) :
