...
class DynamicsNet(nn.Module):
    ...
    def __init__(
        ...
        self.fc = MLP(conv_dim, support_dim, fc_reward_h_dim)
         # or 1 for the res
        self.new_conc_dim = 128
        self.varfc = torch.nn.Linear(self.conv_dim,1) # or also self.new_conc_dim
...
class MuZeroResidualNetwork(AbstractNetwork):
    ...
    def dynamics(self, encoded_state, action):
        ...
        reward, next_encoded_state_mean, next_encoded_state_std  = self.dynamics_network(s_a) #, next_encoded_state_std
        d = next_encoded_state_mean
        b = torch.flatten(d)  # d.view(-1, torch.prod(torch.tensor(d.size())))
        std = torch.flatten(next_encoded_state_std)
        stds = []
        
        for s in std :
            for i in range( torch.prod(torch.tensor(next_encoded_state_mean.size()[1:] )) ) :
                stds.append(s)
        stds = torch.tensor(stds)
        covariance_matrix = torch.diag(torch.square(stds))
        
        next_encoded_state = MultivariateNormal(b, covariance_matrix).sample()
        next_encoded_state = torch.reshape(next_encoded_state, d.size() )
        next_encoded_state_normalized = self.scale(next_encoded_state_mean)
