...
This script defines the SelfPlay class for the MuZero algorithm.

    https://arxiv.org/pdf/1911.08265.pdf

It also contains an implementation of the standard and robust variant of
Monte Carlo Tree Search. In the robust variant, K trajectories are sampled at
each node, and the one with the lowest reward is selected.
...
@ray.remote
class SelfPlay:
    """Class which run in a dedicated thread to play games and save them
    ...
    def __init__(self, initial_checkpoint, Game, config, seed):
        ...
        self.model = network.MuZeroNetwork(self.config)
...
class MCTS:
    ...
    def run(
        ...
        for _ in range(self.config.num_simulations):
            ...
            value, reward, policy_logits, encoded_state, hidden_state = model.recurrent_inference(
                parent.encoded_state,
                torch.tensor([[action]]).to(parent.encoded_state.device),
                parent.hidden_state
            )
    ...
    def ucb_score(self, parent, child, min_max_stats):
        """The score for a node is based on its value, plus an exploration
        bonus based on the prior.
        """
        pb_c = (
            math.log(
    ...
    def backpropagate(self, search_path, value, to_play, min_max_stats):
        """At the end of a simulation, we propagate the evaluation all the way
...
class RobustMCTS(MCTS):
    """Robust version of MCTS

    Sample K trajectories at each node and pick the one with the lowest reward
    """
    def __init__(self, config):
    ...
    def run(
        ...
        for _ in range(self.config.num_simulations):
            ...
            # Sample K trajectories and pick the one with the lowest reward
            value = None
            reward = numpy.inf
            policy_logits = None
            encoded_state = None
            hidden_state = None
            for _ in range(K):
                v, r, p, s, h = model.recurrent_inference(
                    parent.encoded_state,
                    torch.tensor([[action]]).to(parent.encoded_state.device),
                    parent.hidden_state
                )
...
class Node:
    ...
    def expand(self, actions, to_play, reward, policy_logits, encoded_state, hidden_state):
    ...
    def add_exploration_noise(self, dirichlet_alpha, exploration_fraction):
        """At the start of each search, we add dirichlet noise to the prior of
...
class GameHistory:
    ...
    def get_stacked_observations(self, index, num_stacked_observations):
        """Generate a new observation with the observation at the index
        position and num_stacked_observations past observations and actions stacked.
        """
